...
class LiGanDeviceDetailController extends GetxController {
  ...
  Future<void> getDataPage() async {
    ...
    map2['deviceCode'] = deviceNo;
    map2['pageNo'] = dataPageNum;
    map2['pageSize'] = 1;
    var result2 = await HhHttp()
        .request(RequestUtils.soilPage, method: DioMethod.get, params: map2);
    EventBusUtil.getInstance().fire(HhLoading(show: false));
    HhLog.d("soilPage -- $map2");
    HhLog.d("soilPage -- $result2");
    if (result2["code"] == 0 && result2["data"] != null) {
      if(result2["data"]["list"]!=null){
        soilModel = result2["data"]["list"][0];
        dataStatus.value = false;
        dataStatus.value = true;
      }
    } else {
      EventBusUtil.getInstance()
          .fire(HhToast(title: CommonUtils().msgString(result2["msg"])));
    }
    ///气象信息数据
    Map<String, dynamic> map3 = {};
    map3['deviceCode'] = deviceNo;
    map3['pageNo'] = dataPageNum;
    map3['pageSize'] = 1;
    var result3 = await HhHttp()
        .request(RequestUtils.weatherPage, method: DioMethod.get, params: map3);
    EventBusUtil.getInstance().fire(HhLoading(show: false));
    HhLog.d("weatherPage -- $map3");
    HhLog.d("weatherPage -- $result3");
    if (result3["code"] == 0 && result3["data"] != null) {
      if(result3["data"]["list"]!=null){
        weatherModel = result3["data"]["list"][0];
        dataStatus.value = false;
        dataStatus.value = true;
      }
    } else {
